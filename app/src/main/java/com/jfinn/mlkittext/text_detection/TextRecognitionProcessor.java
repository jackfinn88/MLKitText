...
public class TextRecognitionProcessor {
    private static String TAG = TextRecognitionProcessor.class.getSimpleName().trim();

    private final FirebaseVisionTextRecognizer detector;

    // should we ignore process(). usually caused by feeding input data faster than
    ...
    protected void onSuccess(@NonNull FirebaseVisionText results, @NonNull FrameMetadata frameMetadata, @NonNull GraphicOverlay graphicOverlay) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            for (int j = 0; j < lines.size(); j++) {
                // show whole line
                TextGraphic textGraphic = new TextGraphic(graphicOverlay, lines.get(j).getText(), lines.get(j).getBoundingBox());
    ...
    private void detectInVisionImage(FirebaseVisionImage image, final FrameMetadata metadata, final GraphicOverlay graphicOverlay) {
        ...
        // start throttling until this frame has been processed
